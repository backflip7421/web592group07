<?php
 ...
 //เชน่ หนา้ home ก็จะบันทึกข ้อมูลไว้ในไฟล์ home.json
	$fbfile = "gs://$appid/olympus.json"; //แก้ตรงนี้--------------------------------------------------------
	$fbdata = array();
 //ถ ้ามีไฟล์ข ้อมูลให้อ่านไฟล์มาแสดงผล
	if(file_exists($fbfile)){
		$fbjson = file_get_contents($fbfile);
		$fbdata = json_decode($fbjson, true);
		echo "<hr>";
		foreach($fbdata as $fb){
			$text = htmlspecialchars($fb['feedbackolympus']); //แก้ตรงนี้--------------------------------------------------------
	...
	if($user){
		include("main_feedback_add_olympus.php"); //แก้ตรงนี้--------------------------------------------------------
	}
?>